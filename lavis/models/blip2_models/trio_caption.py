...
class TrioT5(Blip2Base):
    ...
    def unpatchify(self, x):
        ...
        """
        p = self.visual_encoder.patch_embed.patch_size[0]
        h = w = int( (x.shape[1]/self.num_frames) **.5)
        assert h * w  * self.num_frames == x.shape[1]
        
        x = x.reshape(shape=(x.shape[0], self.num_frames, h, w, p, p, 3))
        x = torch.einsum('nfhwpqc->nfchpwq', x)
        imgs = x.reshape(shape=(x.shape[0], self.num_frames, 3, h * p, h * p))
        return imgs
