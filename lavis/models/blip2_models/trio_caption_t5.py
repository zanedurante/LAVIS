...
class TrioT5(Blip2Base):
    ...
    def __init__(
        ...
        freeze_qformer=True,
        vit_model_ckpt=None,
        qformer_model_ckpt=None,
        masking_ratio=0.00,
        temporal_freeze_vit=False,
        freeze_nothing=False,
    ):
        """
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder( # use_grad_checkpoint for vit_checkpoint
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision
        )

        self.visual_encoder.set_masking_ratio(masking_ratio)
    
        CLIP_INIT_LAYERS = [
            "patch_embed",
            "patch_embed.proj.weight" # For compatibility with eva-clip models
            "patch_embed.proj.bias"
            "first_frame_temporal_embed",
            "norm_pre",
            "norm1",
            "attn",
            "drop_path",
        ...
        if freeze_vit and temporal_freeze_vit:
            for name, param in self.visual_encoder.named_parameters():
                param.requires_grad = False
            self.visual_encoder.eval()
        elif freeze_vit:
            # Only freeze the spatial layers            
            for name, layer in self.visual_encoder.named_children():
                if name == "blocks":
                    for block in layer:
            ...
            logging.info("spatially freeze vision encoder")
        ...
        if qformer_model_ckpt is not None:
            print("Loading qformer parameters from checkpoint: ", qformer_model_ckpt)
            self.Qformer.load_state_dict(torch.load(qformer_model_ckpt))
    ...
    def from_config(cls, cfg):
        ...
        freeze_nothing = cfg.get("freeze_nothing", False)
        ...
        vit_model_ckpt = cfg.get("load_vit_ckpt", None)
        qformer_model_ckpt = cfg.get("load_qformer_ckpt", None)

        freeze_qformer = cfg.get("freeze_qformer", True)
        masking_ratio = cfg.get("masking_ratio", 0.75)
        temporal_freeze_vit = cfg.get("temporal_freeze_vit", False)
        ...
        model = cls(
            ...
            qformer_model_ckpt=qformer_model_ckpt,
            freeze_qformer=freeze_qformer,
            masking_ratio=masking_ratio,
            freeze_nothing=freeze_nothing,
        )
